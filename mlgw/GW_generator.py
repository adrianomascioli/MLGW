...
class mode_generator_NN(mode_generator_base):
	...
	def load(self, folder, verbose=False, batch_size=10):
		if not os.path.isdir(folder):
			raise RuntimeError(f"Unable to load folder {folder}: no such directory!")
		...
		for q_str in ['amp', 'ph']:
			for nn_file in glob.glob(str(folder / f"{q_str}*[0-9]*h5")):
				...
				if 'residual' in nn_file:
					...
					try:
						self.ph_res_coefficients[comps] = np.loadtxt(folder / f"residual_coefficients_{comps}")
					except FileNotFoundError:
						warnings.warn(f"Missing residual coefficients for '{nn_file}': skipping residual net.")
						continue
				else:
					comps = re.findall(r'_[0-9]+\.', nn_file)
					assert len(comps) == 1, f"Invalid NN filename: {nn_file}"
					comps = comps[0][1:-1]
				...
				def wrapped_model(theta, features, *, _jax_fn=jax_fn, _params=params):
					input_ = augment_features(theta, features)
					return _jax_fn(_params, input_)
				'''
		
		
				dict_to_fill[comps] = {
                    "model": jax_fn,
                    "augment_features": augmenter,
                    "params": params
                }
